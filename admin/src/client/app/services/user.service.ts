...
export class UserService {
  ...
  getAllUsers(search='',sort='',limit=0) {
    var query = '?search='+search+'&sort='+sort+'&limit='+limit;
    return this.http.get('http://localhost:8080/user'+query)
      .map(res => res.json());
  }

  removeUser(id:string) {
    return this.http.delete('http://localhost:8080/user/'+id, {withCredentials: true})
      .map(res => res.json());
  }
}
